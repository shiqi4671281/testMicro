...
func InitDb()(*gorm.DB,error){
	//sql.Open()
	db,err := gorm.Open("mysql",viper.GetString("MYSQLLOGIN")+"/"+viper.GetString("MYSQLDB")+"?charset=utf8&parseTime=True&loc=Local")
	Gdb=db
	if err == nil {
		Gdb.DB().SetMaxIdleConns(3)
		Gdb.DB().SetConnMaxLifetime(30)
		Gdb.SingularTable(true)
		Gdb.AutoMigrate(new(User),new(House),new(Area),new(Facility),new(HouseImage),new(OrderHouse))
